...
void trie_insert_string(struct trie *t, const char *str)
{
	...
	if (t->branch_count == 0) {
		...
		/* insert key logic */
		t->branch_count++;
		t->branches = (struct trie_branch **)realloc(t->branches,
			sizeof(struct trie_branch *) * t->branch_count);
		t->branches[t->branch_count - 1] = (struct trie_branch *)
			malloc(sizeof(struct trie_branch));
		if (t->branches[pos]->key < *str) {
...
static void trie_branch_insert_string(struct trie_branch *b, const char *str)
{
	...
	if (b->branch_count == 0) {
		...
		/* insert key logic */
		b->branch_count++;
		b->branches = (struct trie_branch **)realloc(b->branches,
			sizeof(struct trie_branch *) * b->branch_count);
		b->branches[b->branch_count - 1] = (struct trie_branch *)
			malloc(sizeof(struct trie_branch));
		if (b->branches[pos]->key < *str) {
